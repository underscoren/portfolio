...
export class MainScene extends Container implements IScene {
    ...
    constructor() {
        ...
        this.on("pointermove", (ev) => {
            if (ev.pointerType == "mouse" || (ev.pointerType == "touch" && ev.pointerId == 0))
                // don't set input using secondary touches
                Input.mouseX = ev.global.x;
        });

        this.on("pointerdown", (ev) => {
            console.log(ev.pointerType, ev.pointerId);
            if (ev.pointerType == "touch" && ev.pointerId == 0) return; // only allow secondary touches to fire

            if (this.powerManager.ammo > 0) {
                this.entitySystem.addEntity(
                    this.powerManager.fireShot(
                        this.paddleBottom.x,
                        this.paddleBottom.y,
                        this.paddleBottom.scale.x
                    )
                );
        ...
        if (screen.width < 768 || screen.height < 768) {
            ...
            this.on("tap", () => {
                if (!document.fullscreenElement) this.mobileux.fullscreenAndRotate();
            });
        }
    ...
    spawnBall(x: number, y: number, velX: number, velY: number) {
        ...
        ball.onBallOffscreen = (top) => {
            ...
            if (BallManager.ballCount - 1 < this.cannonManager.minBalls) {
                // this is technically called before the ball is removed, need to account for 1 fewer ball
    ...
    spawnCannon(x: number | void, delay: number | void) {
        ...
        if (delay) cannon.lifetime = -delay;

        cannon.onSpawnBall = () => this.spawnBall(cannon.x, cannon.y - BALL_RADIUS, 0, -BALL_SPEED);
