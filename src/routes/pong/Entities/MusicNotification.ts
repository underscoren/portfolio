...
export class MusicNotification extends Entity {
    ...
    update() {
        ...
        if (this.lifetime <= RISE_TIME) {
            this.x =
                SCREEN_WIDTH - lerp(0, this.targetWidth, easeOutBack(this.lifetime / RISE_TIME));
        }
        ...
        if (this.lifetime > STAY_TIME_CUMUL && this.lifetime <= FALL_TIME_CUMUL) {
            this.x =
                SCREEN_WIDTH -
                lerp(this.targetWidth, 0, (this.lifetime - STAY_TIME_CUMUL) / FALL_TIME);
        }

        if (this.lifetime > FALL_TIME_CUMUL) EntitySystem.delete(this);
    }
}
